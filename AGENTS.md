...
- **Enforcement:** the server-owned enforcer in `src/server/services/concurrentSessionEnforcer.ts` is called from `/exam/questions`, answer-save routes, and `/violation`. On `suspicious` it appends a forensic `concurrent_session` row to `violation_events` (`metadata_json` = `{ ips, userAgents, jtis, overlap }`, `content_preview` = the IP list); on `lockable` (still `in_progress`) it additionally auto-submits and returns `410 { reason: 'concurrent_session' }` (questions/answers) or `locked: true` (violation). This lock is direct and does not depend on violation counters. Client-supplied `concurrent_session` values are rejected.
