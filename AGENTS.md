...
- **DB schema:** recording metadata uses `recording_parts`, `recording_upload_reservations`, `students.recording_manifest_sealed_at`, `students.recording_expected_part_count`, and the per-attempt frozen `students.attempt_record_mode`; Live adds `batches.live_enabled` plus the `live_monitor_audit` table. The VMware gate adds `batches.vmware_check_enabled` and `students.environment_check_passed`, `environment_snapshot`, `environment_checked_at`. Local/fresh bootstrap creates the full schema. Production must apply `20260827_recording_upload_reservations.sql`, `20260827_recording_manifest_recovery.sql`, `20260828_live_monitoring.sql`, `20260829_live_enabled.sql`, `20260829_vmware_environment_check.sql`, then `20260829_vmware_environment_check_default_off.sql` after the 20260819 cleanup migration before deploying schema-v7 code.
...
- Show Live only for the creating user, batches whose end calendar date is not before today, and batches with an actual capture source (`local`, `s3`, or `live_enabled`). `migrations/20260829_live_enabled.sql` establishes the schema-v6 Live step and `20260828_live_monitoring.sql` supplies the signaling/audit policy; production source now also requires the two VMware migrations that raise the contract to v7.
