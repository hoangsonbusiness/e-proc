...
- Internal diagnostic endpoints (`/api/test-db`, `/api/queue/stats`, `/api/cache/flush`, `/api/stats`) require admin JWT. `/api/queue/process` accepts either admin JWT or the exact `CRON_SECRET`. `/api/init-tables` has been removed — DB init runs automatically on server startup.
