...
const SQL_LOCKABLE_TOTAL = `SELECT SUM(count) as total FROM violations
...
export const SQL_INSERT_EVENT = `INSERT INTO violation_events (student_id, batch_id, type, text_length, content_preview, question_id, metadata_json, event_id)
   VALUES (?, ?, ?, ?, ?, ?, ?, ?)
   ON CONFLICT (student_id, event_id) WHERE event_id IS NOT NULL DO NOTHING`;
export const SQL_UPSERT_COUNTER = `INSERT INTO violations (student_id, type, count) VALUES (?, ?, 1)
   ON CONFLICT (student_id, type) DO UPDATE SET count = violations.count + 1`;
export async function persistViolation(tx, input) {
 ...
 */
export function computeViolationLock(type, currentCount, total, forensicOnly) {
