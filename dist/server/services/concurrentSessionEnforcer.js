 ...
 */
export function createConcurrentSessionEnforcer({ db, detect, submit, now = Date.now, logger = console, }) {
    const lastForensic = new Map();
    const inflightForensic = new Set();
    return async function enforceConcurrentSession(studentId, batchId) {
        try {
            ...
            const shouldLog = !inflightForensic.has(dedupKey) &&
            ...
            if (shouldLog) {
                ...
                try {
                    ...
                    await db.query('INSERT INTO violation_events (student_id, batch_id, type, text_length, content_preview, question_id, metadata_json) VALUES (?, ?, ?, ?, ?, ?, ?)', [
                        studentId,
                        batchId,
                        'concurrent_session',
                        evidence.ips.length,
