...
function violationBreakdowns(rows) {
    const grouped = new Map();
    for (const row of rows) {
        const studentId = Number(row.student_id);
...
function violationTotal(breakdown) {
    return Object.values(breakdown).reduce((sum, count) => sum + count, 0);
}
export async function loadBatchResultsSummary(db, batchId, pagination) {
  ...
  `, [batchId, pagination.pageSize, offset]);
    ...
    const studentIds = studentsResult.rows.map((student) => Number(student.id));
    ...
    const eventSummaries = new Map(eventSummaryResult.rows.map((row) => [Number(row.student_id), row]));
    const recordingSummaries = new Map(recordingSummaryResult.rows.map((row) => [Number(row.student_id), row]));
    const items = studentsResult.rows.map((student) => {
        const studentId = Number(student.id);
        const breakdown = breakdowns.get(studentId) || {};
        const eventSummary = eventSummaries.get(studentId);
        const recordingSummary = recordingSummaries.get(studentId);
        return {
            student: {
                ...student,
                id: studentId,
...
export async function loadBatchResultsLegacy(db, batchId) {
...
export async function loadBatchExportData(db, batchId) {
    ...
    const questions = rowsByStudent(questionsResult.rows);
    const violations = new Map(violationsResult.rows.map((row) => [Number(row.student_id), Number(row.total) || 0]));
    return studentsResult.rows.map((student) => ({
        student,
