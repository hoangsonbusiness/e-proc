import { persistViolation, } from './violationStore.js';
/**
 ...
 * `in_progress`.
 */
export async function persistViolationIfInProgress(tx, input) {
    const statusResult = await tx.query(`SELECT status FROM students WHERE id = ?${input.lockStudentRow ? ' FOR UPDATE' : ''}`, [input.studentId]);
