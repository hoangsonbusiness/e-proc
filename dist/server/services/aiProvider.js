...
export function normalizeConnectionConfig(input, apiKey = '') {
    const provider = String(input?.provider || '').trim().slice(0, 100);
    const apiProtocol = String(input?.apiProtocol || input?.api_protocol || '');
    ...
    if (!AI_PROTOCOLS.includes(apiProtocol))
        throw new Error('Unsupported API protocol');
    if (!baseUrl)
        throw new Error('Base URL is required');
    if (!model)
        throw new Error('Model is required');
    if (!apiKey && apiProtocol !== 'ollama_generate')
        throw new Error('API key is required');
    return { provider, apiProtocol, baseUrl, apiKey, model };
}
export async function assertSafeProviderUrl(rawUrl) {
    let url;
    try {
        url = new URL(rawUrl);
    }
    ...
    if (production && url.protocol !== 'https:')
        throw new Error('Base URL must use HTTPS in production');
...
function safeProviderError(status, _body) {
...
async function readResponse(response) {
    ...
    if (Buffer.byteLength(text, 'utf8') > MAX_RESPONSE_BYTES)
        throw new Error('LLM response is too large');
    if (!response.ok)
        throw safeProviderError(response.status, text);
    try {
...
export async function callLlm(config, request) {
    ...
    if (response.status >= 300 && response.status < 400)
        throw new Error('LLM redirects are not allowed');
    ...
    if (config.apiProtocol === 'openai_responses') {
        ...
        const texts = (data?.output || []).flatMap((item) => item?.content || []).map((item) => item?.text).filter(Boolean);
        return texts.join('\n');
    }
    ...
    if (config.apiProtocol === 'gemini_generate_content') {
        return (data?.candidates?.[0]?.content?.parts || []).map((item) => item?.text).filter(Boolean).join('\n');
    }
