...
import { ReadinessController, isPermanentDatabaseStartupError, } from './readiness.js';
dotenv.config();
...
async function initPostgres() {
    ...
    try {
        ...
        if (!allowRuntimeBootstrap) {
            throw new Error(`[schema] expected app_schema_state version >= ${MINIMUM_SCHEMA_VERSION}, got ${installedSchemaVersion ?? '<missing>'}. Run migrations before serving.`);
        }
...
async function disposeDatabaseAfterFailedStartup() {
    if (USE_SQLITE) {
        if (sqliteDb?.open)
            sqliteDb.close();
        sqliteDb = null;
...
const retryMaxDelayMs = Math.max(retryBaseDelayMs, parseInt(process.env.DB_READY_RETRY_MAX_MS || '30000') || 30000);
