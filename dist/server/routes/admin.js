...
import { AiGradingError, gradeBatchManually, gradeSelectedStudentsManually, gradeStudentManually, } from '../services/batchAiGrading.js';
...
router.get('/is-initialized', async (req, res) => {
    try {
...
router.post('/setup', setupRateLimit, async (req, res) => {
    try {
        ...
        await db.query('INSERT INTO admin_users (username, password_hash) VALUES (?, ?)', [username.trim(), passwordHash]);
...
router.post('/login', loginRateLimit, async (req, res) => {
    try {
        ...
        const result = await db.query('SELECT * FROM admin_users WHERE username = ?', [username.trim()]);
        ...
        const token = jwt.sign({ id: user.id, username: user.username, role }, secret, { expiresIn });
...
router.get('/users', requireAdmin, async (_req, res) => {
    try {
        const result = await db.query('SELECT id, username, role, created_at FROM admin_users ORDER BY id ASC');
...
router.post('/users', requireAdmin, async (req, res) => {
    try {
        ...
        const passwordHash = await bcrypt.hash(password, 10);
        await db.query('INSERT INTO admin_users (username, password_hash, role) VALUES (?, ?, ?)', [username.trim(), passwordHash, role]);
...
router.put('/change-password', async (req, res) => {
    try {
        ...
        const result = await db.query('SELECT * FROM admin_users WHERE id = ?', [adminUser.id]);
        const user = result.rows[0];
        if (!user) {
        ...
        await db.query('UPDATE admin_users SET password_hash = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [newHash, adminUser.id]);
...
router.get('/questions/type-stats', async (req, res) => {
    try {
        const result = await db.query(`
      SELECT
        type,
...
router.get('/questions/module-type-stats', async (req, res) => {
    try {
        const result = await db.query(`
      SELECT
        module,
    ...
    try {
        ...
        if (req.adminUser?.role !== 'admin') {
            ...
            if (USE_SQLITE) {
                ...
                const owned = await db.query(`SELECT id FROM question_bank WHERE id IN (${placeholders}) AND uploaded_by = ?`, [...ids, userId]);
                ...
                const owned = await db.query(`SELECT id FROM question_bank WHERE id = ANY($1::text[]) AND uploaded_by = $2`, [ids, userId]);
                const ownedIds = new Set(owned.rows.map((r) => r.id));
                const forbidden = ids.filter(id => !ownedIds.has(id));
                if (forbidden.length > 0) {
                    return res.status(403).json({ error: 'Forbidden: You can only delete questions you uploaded' });
...
router.put('/batches/:id', async (req, res) => {
    try {
        ...
        const { name, start_time, end_time, duration, blueprint, record_mode, vmware_check_enabled, live_enabled, exam_type } = req.body;
        const examType = exam_type === 'quiz' ? 'quiz' : 'essay';
        const currentResult = await db.query('SELECT created_by, record_mode, vmware_check_enabled, live_enabled FROM batches WHERE id = ?', [batchId]);
...
router.post('/batches/:id/ai-grade', async (req, res) => {
    try {
        const batchId = Number(req.params.id);
        const userId = req.adminUser?.id;
        if (!Number.isInteger(batchId) || batchId < 1)
...
router.post('/batches/:batchId/students/ai-grade', async (req, res) => {
    try {
        const batchId = Number(req.params.batchId);
        const userId = req.adminUser?.id;
        if (!Number.isInteger(batchId) || batchId < 1)
