...
const isDbTrue = (value) => value === true || value === 1 || value === '1';
function safeRecordingErrorCode(error) {
...
function logRecordingOperation(input) {
    ...
    if (input.statusCode >= 500)
        ...
        console.info('[recording]', event);
}
function recordingUploadId(req, requestedPartIndex) {
...
function isValidRecordingUploadId(uploadId) {
...
function readRecordingRecoveryStatusLocked(studentId, batchId) {
...
async function submitExamAtomically(studentId, reason, options = {}) {
    ...
    `;
       ...
       WHERE id = ? AND status = 'in_progress'`, [new Date().toISOString(), finalReason, recordMode, recordMode, studentId]);
...
async function startExamAtomically(studentId) {
    return db.withTransaction(async (tx) => {
       ...
       WHERE s.id = ?${USE_SQLITE ? '' : ' FOR UPDATE'}`, [studentId])).rows[0];
        ...
        if (locked.status === 'in_progress' && existingCount > 0) {
         ...
         SET disconnected_at = NULL,
             attempt_record_mode = COALESCE(attempt_record_mode, ?)
         WHERE id = ?`, [attemptRecordMode, studentId]);
       ...
       attempt_record_mode = COALESCE(attempt_record_mode, ?) WHERE id = ?`, [now.toISOString(), deadline.toISOString(), attemptRecordMode, studentId]);
...
router.post('/exam/environment-check', studentAuthMiddleware, async (req, res) => {
    try {
        res.set('Cache-Control', 'no-store, no-cache, must-revalidate, proxy-revalidate');
        const studentId = req.studentPayload.studentId;
        const current = (await db.query(`SELECT s.status, b.vmware_check_enabled
       FROM students s JOIN batches b ON b.id = s.batch_id
       WHERE s.id = ?`, [studentId])).rows[0];
        if (!current)
            return res.status(404).json({ error: 'Student not found' });
        if (current.status === 'submitted')
            return res.status(410).json({ error: 'Exam already submitted', reason: 'submitted' });
        if (!isDbTrue(current.vmware_check_enabled))
            return res.json({ success: true, enforced: false });
       ...
       WHERE id = ?`, [USE_SQLITE ? (decision.allowed ? 1 : 0) : decision.allowed, JSON.stringify(metadata), checkedAt, studentId]);
...
router.post('/exam/start', studentAuthMiddleware, async (req, res) => {
    try {
      ...
      `, [student_id, q.id, i + 1, optionOrder]);
       ...
       WHERE id = ?`, [now.toISOString(), deadline.toISOString(), student_id]);
...
router.post('/violation', studentAuthMiddleware, sessionTracker, async (req, res) => {
    try {
        ...
        const { ignored, replay, total, currentCount } = await db.withTransaction((tx) => persistViolationIfInProgress(tx, {
...
router.post('/exam/recording-seal', studentAuthMiddleware, async (req, res) => {
    const startedAt = Date.now();
    const studentId = req.studentPayload.studentId;
    const batchId = req.studentPayload.batchId;
...
router.get('/exam/recording-status', studentAuthMiddleware, async (req, res) => {
    const startedAt = Date.now();
    const studentId = req.studentPayload.studentId;
    const batchId = req.studentPayload.batchId;
    let outcome = 'read';
...
router.post('/exam/recording-reconcile', studentAuthMiddleware, async (req, res) => {
    const startedAt = Date.now();
    const studentId = req.studentPayload.studentId;
    const batchId = req.studentPayload.batchId;
    let outcome = 'rejected';
    ...
    try {
        const status = await db.withTransaction(async (tx) => {
            ...
            if (current.expectedPartCount > 0
...
router.post('/exam/recording-complete', studentAuthMiddleware, async (req, res) => {
    ...
    try {
        ...
        if (!Number.isSafeInteger(acknowledgedByteSize)
