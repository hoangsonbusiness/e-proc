...
import { callLlm, connectionFingerprint, normalizeConnectionConfig, type LlmConnectionConfig } from './aiProvider.js';
...
export async function loadAiSettingRow(db: DbExecutor, userId: number): Promise<any | null> {
  const result = await db.query('SELECT * FROM user_ai_settings WHERE user_id = ?', [userId]);
  return result.rows[0] || null;
}

export async function getOwnedAiSetting(db: DbExecutor, userId: number): Promise<any> {
  const row = await loadAiSettingRow(db, userId);
  if (!row) return { configured: false, testStatus: 'not_configured', hasApiKey: false };
  return {
    configured: true,
    id: Number(row.id),
...
async function resolveDraftConfig(db: DbExecutor, userId: number, input: any): Promise<LlmConnectionConfig> {
  ...
  if (!apiKey) {
    ...
    if (existing?.encrypted_api_key) apiKey = decryptSecret(existing);
  }
  return normalizeConnectionConfig(input, apiKey);
}

export async function testOwnedAiSetting(db: DbExecutor, userId: number, input: any): Promise<any> {
  ...
  if (!response.trim()) throw new Error('LLM returned an empty response');
  const fingerprint = connectionFingerprint(config);
  const testToken = jwt.sign(
    { purpose: 'ai-setting-test', userId, fingerprint },
    jwtSecret(),
    { expiresIn: '10m' },
  );
  return {
...
export async function saveOwnedAiSetting(db: DbExecutor, userId: number, input: any): Promise<any> {
  const config = await resolveDraftConfig(db, userId, input);
...
export async function loadVerifiedConnection(db: DbExecutor, userId: number, settingId?: number): Promise<LlmConnectionConfig> {
  const row = await loadAiSettingRow(db, userId);
  ...
  if (connectionFingerprint(config) !== row.tested_config_hash) throw new Error('AI setting must be tested again');
  return config;
}
