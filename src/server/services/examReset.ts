...
export async function reopenExamAttempt(
  ...
  const student = (await tx.query(
    `SELECT s.id, s.status, b.end_time
     FROM students s JOIN batches b ON b.id = s.batch_id
     WHERE s.id = ?${lockStudentRow ? ' FOR UPDATE' : ''}`,
    [studentId],
  )).rows[0];
  if (!student) throw new ExamResetError(404, 'Student not found');

  const questionsCount = Number((await tx.query(
