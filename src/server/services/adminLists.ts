...
export async function loadPagedBatches(
  db: DbExecutor,
  options: PaginationOptions & { includeBlueprint: boolean },
): Promise<any> {
  ...
  const items = itemsResult.rows.map((batch: any) => ({
    ...batch,
...
export async function loadPagedStudents(
  db: DbExecutor,
  batchId: number,
