...
- **Known bug (unfixed as of this writing):** `query()` in `src/server/db/postgres.ts` decides how to run a SQLite statement purely by checking `text.trim().toUpperCase().startsWith('SELECT')` — anything else (including `INSERT ... RETURNING id`) goes through `stmt.run(...)`, which always returns `rows: []`. This silently breaks any `INSERT ... RETURNING` under local SQLite dev: `POST /batches/:id/students/import` in `admin.ts` reads `studentResult.rows[0]?.id` after such an insert and gets `undefined`, so `if (!studentId) continue;` skips exam_questions assignment for every invited student (student row is created, but with zero questions). Confirmed via manual testing (`Student created: undefined` in server logs). Postgres is unaffected (its branch always returns real rows). Before relying on "invite students" working end-to-end in local SQLite dev, check whether this has been fixed — the fix is to detect `RETURNING` in the SQL text (case-insensitive) and route it through `.all()`/`.get()` instead of `.run()`.
