...
- **Run code (self-check)**: `POST /api/student/run` (`src/server/coderunner.ts`) compiles & runs the student's current answer so they can check correctness before submitting — wired to the "▶ Run Code" button on `/practice`. Supported: `c` (gcc), `cpp` (g++), `python`, `cobol` (cobc, tries `-free` then falls back to fixed-format), `java` (filename derived from the `public class` name). Resource guards (t3.micro has only 1GB RAM shared with the app): max 2 concurrent runs server-wide with a bounded wait queue, 1 run per student at a time, compile timeout 10s / run timeout 5s with process-group SIGKILL, Linux ulimits (256MB vmem / 64 procs / 10MB files), output capped at 64KB, per-run temp dir deleted afterwards. Student processes get a **clean env** (`PATH` only — never `DATABASE_URL`/`JWT_SECRET`). Only `in_progress` students can run. This is NOT a hard sandbox (student code runs as the app user) — acceptable for internal training; move to Docker/nsjail/Judge0 before any public-facing use. Compilers must be installed on the EC2 box once: `sudo apt install -y gcc g++ gnucobol default-jdk python3` — a missing compiler returns a clear `Cannot start "gcc"...` error rather than crashing.
