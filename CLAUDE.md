...
- **DB schema:** recording recovery uses `recording_parts`, `recording_upload_reservations`, `students.recording_manifest_sealed_at`, `students.recording_expected_part_count`, and frozen `students.attempt_record_mode`. Production must apply both `20260827` recording migrations before schema-v4 code.
- **Reset behavior:** resetting a student deletes recording metadata/reservations and clears `attempt_record_mode`, but does **not** delete existing S3 objects. A new attempt uses a different session-hash namespace, so stale URLs cannot overwrite it; old objects remain until Lifecycle expiry.
