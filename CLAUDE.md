...
- `question_bank` columns relevant to import/grading/export (see "Question bank: groups and HTML-safe plain text" below):
  ...
  - `question_code_sample` / `question_console_output` — both nullable, for Practice-style long-form questions (e.g. `Practice_M3.docx`-style exams) that ship starter/reference code and an expected console output alongside the task description. When either is set, `StudentExam.tsx` renders a two-column layout (task description left, code panels right, via `.question-two-col` in `global.css`) instead of the single-column view; ordinary questions that don't set these render exactly as before. Both are plain text (not HTML — rendered as literal `<pre>` content, not sanitized/parsed), and are included in the AI grading prompt (`src/server/cache.ts`) so grading accounts for the given code/expected output, not just the prose task.
  - `admin_users`
...
- Excel header columns for Practice-style two-part questions (optional): `CodeSample` (aliases: `Code Mẫu`, `Starter Code`) and `ConsoleOutput` (aliases: `Console Output`, `Kết quả Console`). Stored verbatim as plain text into `question_code_sample`/`question_console_output` — no HTML stripping is applied since these are already plain code, not prose. Leave both columns absent/blank for ordinary single-part questions.
